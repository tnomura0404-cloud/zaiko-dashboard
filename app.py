...
# --------------------------------------------------------------------------------
# ★★★ あなたが作成した「v7.9」のExcel整形関数を、そのまま移植 ★★★
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# ★★★ あなたが作成した「v7.9」の分析ロジックをベースにした関数 ★★★
# --------------------------------------------------------------------------------
...
def to_excel_original(brand_groups, low_auto, low_manual, long_term, df_auto, df_manual, df_long):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        if not df_auto.empty:
            df_auto_sorted = df_auto.sort_values(by=["ブランド", "商品名"])[["ブランド", "商品名", "在庫数", "基準数量(自動)", "基準数量(手動)", "差し引き数量"]]
        ...
        if not df_manual.empty:
            df_manual_sorted = df_manual.sort_values(by=["ブランド", "商品名"])[["ブランド", "商品名", "在庫数", "基準数量(自動)", "基準数量(手動)", "差し引き数量"]]
        ...
        if not df_long.empty:
            ...
            grand_total = pd.to_numeric(df_long_sorted['合計金額'], errors='coerce').sum()
            if grand_total > 0:
                next_row = ws.max_row + 2
                ws.cell(row=next_row, column=6, value="総合計金額").font = Font(bold=True)
                total_cell = ws.cell(row=next_row, column=7, value=grand_total)
                total_cell.number_format = '#,##0.00'; total_cell.font = Font(bold=True)
        ...
        for brand in sorted(brand_groups.keys()):
            ...
            df_brand_cleaned = df_brand.drop(columns=['brand', 'INVENTORY_LEVEL', '贩卖单价'], errors='ignore')
            ...
            try:
                ...
                for r_idx, row in enumerate(df_brand.itertuples(), 2):
                    p_name = str(getattr(row, '商品名称', ''))
